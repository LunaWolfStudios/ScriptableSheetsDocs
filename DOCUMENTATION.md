...
- **Scan**: Settings for scanning Objects.
...
- **Table Nav**: Settings for navigating the table view.
...
## External Tools
- **Google Sheets**: Google Sheets uses '\t' delimiter for columns by default. If you are using a different delimiter, then use the split across cells option in Google Sheets when pasting content from Scriptable Sheets.
- **Line Endings**: Scriptable Sheets will default to using the Systems new lines '\r\n' for Windows, '\r' for Mac, and '\n' for Linux. Use a text editor like Notepad to change line endings to match your row delimiter setting before pasting content across cells. The Paste Pad Editor Window will default to '\n' for new lines entered directly. It will copy/paste '\r\n' accordingly.
- **Variable Expansion**: If "Use Expansion" is enabled, you can use variable expansion when naming newly created ScriptableObjects, such as `{i}` for index and `{t}` for type. The index will reset each time you create new assets.
...
- **Large Arrays**: Large arrays will slow down performance. Consider moving larger arrays into a separate ScriptableObject and referencing that in your parent Object to improve performance. Under the Collectiond sample see how a StringCollection is created from a base ScriptableCollection.
