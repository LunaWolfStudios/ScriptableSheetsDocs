...
# Limitations
This section covers what Scriptable Sheets cannot do at this time, whether due to a performance trade-off, a feature that is not yet supported, or a limitation within Unity's own API.
...
- **Collection Tables**: A collection nested inside another collection's elements cannot be expanded into a Collection Table, since it cannot resolve to a single table without choosing a parent element; isolate it in its own ScriptableObject if you need it as a table. Collection Tables are also not recommended for very large or deeply nested collections. While a Collection Table is active, Auto Generate and the global create button do not apply, and the read-only Group Name and Index columns can never rename an owner, reindex an element, or add a new row on import, copy, or paste. See the [FAQ](#faq) for how Collection Tables work.
...
- **How do I show asset previews and icons?** Enable "Show Asset Previews" under [User Interface](#user-interface-settings) and increase the "Row Line Height" so the preview has room. "Asset Preview Scale Mode" controls how the image fits.
- **How do I view GUIDs, asset paths, and read-only fields?** Enable "Show GUID", "Show Asset Path", and "Show Read-only" under [User Interface](#user-interface-settings). "Show Read-only" also reveals the internal values of Addressable Asset References.
- **How do I work with arrays?** Enable "Show Children" and then "Show Arrays" under [User Interface](#user-interface-settings) to show array elements as columns, and use "Override Array Size", "Best Fit", and "Array Min Size" to control how many element columns appear. To instead flip a single array or list into its own table of rows, enable Collection Tables (see below). Large arrays hurt performance, so consider moving them into their own ScriptableObject — see [Performance](#performance) and the Collections sample. Copying an entire array from the Inspector and pasting it across cells is not supported; use the copy row button instead.
- **How do I display multiline text?** Increase the "Row Line Height" under `Settings -> User Interface` to make room for multiline strings and larger asset previews.
...
- **How do I automatically create Objects when importing or pasting?** By default Scriptable Sheets will not create new Objects on import or paste, so any rows beyond the existing Objects are discarded. Enable "Auto Generate" under [Object Management](#object-management-settings) to automatically create enough ScriptableObjects to match the row count before parsing. It only applies to creatable ScriptableObject types and only adds Objects, never deletes them. With Auto Generate disabled, create the Objects ahead of time before importing.
- **How do I search and filter Objects?** The search bar filters by name by default. You can also filter by GUID, asset path, and any property using advanced expressions, including chained filters and column references. See [Advanced Search Filtering](#advanced-search-filtering).
- **Can I use formulas?** No, there are no formulas for filling cells.
